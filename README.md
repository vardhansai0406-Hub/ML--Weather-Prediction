...
cols_to_fill = ['tem', 'pm2_5', 'tsr', 'hum', 'pressure', 'wind_speed', 'illumination',
'co2']
...
processed_df = df.dropna(subset=['tem_lag2', 'pm2_5_lag2', 'tsr_lag2', 'hum',
...
axes[i, 0].plot(res['actual'][-150:], label='Actual', color='black', alpha=0.4,
linewidth=2)
axes[i, 0].plot(res['preds'][-150:], label='Predicted', color=color, linestyle='--',
linewidth=2)
...
importances = pd.Series(models[target].feature_importances_,
...
'hum': last_row['hum'], 'pressure': last_row['pressure'], 'wind_speed':
last_row['wind_speed'],
'illumination': last_row['illumination'], 'co2': last_row['co2'],
'hour_sin': last_row['hour_sin'], 'hour_cos': last_row['hour_cos'],
'tem_lag1': last_row['tem'], 'pm2_5_lag1': last_row['pm2_5'], 'tsr_lag1':
last_row['tsr']
}])
print("\n--- NEXT STEP PREDICTIONS (Using Latest Data) ---")
for target in targets:
pred_val = models[target].predict(latest_data)[0]
print(f"Predicted {target_meta[target]
```

## Output:
<img width="706" height="543" alt="image" src="https://github.com/user-attachments/assets/342cce55-e9b4-476a-a6e3-a0c4a2e1bcc7" />


## Result: Hence the  Implementation of Random Forest Algorithm for Weather Prediction has been sucessfully executed the output 
